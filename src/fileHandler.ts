  ...
  try {
    ...
    if (error instanceof Error) {
      if ("code" in error && error.code !== "ENOENT") {
        logger.error(
          `Error reading .gitignore file ${filePath}: ${error.message}`
        );
      }
    } else {
      logger.error(
        `Unknown error reading .gitignore file ${filePath}: ${String(error)}`
      );
    }
    return new Set();
  }
...
async function createOutputDirectory() {
  try {
    ...
    logger.info(
      `Output directory created or already exists: ${config.outputDir}`
    );
  } catch (error) {
    const errorMsg = `Error creating output directory: ${error}`;
    logger.error(errorMsg);
    throw error;
  }
}
